...
{
    public sealed class TreasureRoomOverlayUIView : MonoBehaviour
    {
        public const string BackgroundResourcePath = "UI/RunMap/treasure_room_bg_v1";
        public const string BackgroundAssetPath =
            "Assets/FracturedChorus/Art/UI/RunMap/Treasure/treasure_room_bg_v1.png";
        public const string BackgroundVideoAssetPath =
            "Assets/FracturedChorus/Art/UI/RunMap/Treasure/treasure_room_bg_v1.mp4";
        ...
        private static readonly Color TitleColor = new Color(1f, 0.86f, 0.42f, 1f);
        ...
        [SerializeField] private TreasureRewardTableSO rewardTable;
        [SerializeField] private bool preserveSceneLayout = true;

        private readonly ChoiceCard[] _cards = new ChoiceCard[MaxCards];
        private Action<TreasureRewardSO> _onPicked;
        private bool _resolved;

        public TreasureRewardTableSO RewardTable => rewardTable;

        public static TreasureRoomOverlayUIView EnsureOnCanvas(Transform canvasRoot)
        {
            if (canvasRoot == null)
            {
                return null;
            ...
            var existing = canvasRoot.GetComponentInChildren<TreasureRoomOverlayUIView>(true);
            if (existing != null)
            {
            ...
            var view = go.GetComponent<TreasureRoomOverlayUIView>();
        ...
        public void Show(TreasureRewardSO[] offers, Action<TreasureRewardSO> onPicked)
        {
            WireSceneReferences();
            _onPicked = onPicked;
            _resolved = false;
            ...
            if (hintText != null)
            {
                hintText.text = "Chọn 1 phần thưởng";
            }
        ...
        public void SetRewardTable(TreasureRewardTableSO table)
        {
            rewardTable = table;
        }
        ...
        public void ShowEditPreview()
        {
            if (rewardTable == null)
            {
                rewardTable = TreasureRewardTableSO.LoadOrCreateDefault();
            }

            var offers = rewardTable != null
                ? rewardTable.PickOffers(1)
                : Array.Empty<TreasureRewardSO>();
            Show(offers, _ => { });
        }
        ...
        public void BuildDefaultHierarchy()
        {
            ...
            titleText = CreateLabel("Title", root, "TREASURE", 52, TextAnchor.MiddleCenter, TitleColor);
            ...
            hintText = CreateLabel("Hint", root, "Chọn 1 phần thưởng", 24, TextAnchor.MiddleCenter, FcColorTokens.Brand.TextMuted);
            ...
            for (var i = 0; i < MaxCards; i++)
            {
                var go = HologramChoiceCardChrome.Create($"ChoiceCard_{i}", root, false);
        ...
        public void ApplyBackground()
        {
            ...
            {
                backgroundVideo = Resources.Load<VideoClip>(BackgroundResourcePath);
            }
            ...
            if (backgroundVideo == null)
            {
                backgroundVideo = AssetDatabase.LoadAssetAtPath<VideoClip>(BackgroundVideoAssetPath);
            }
#endif

            var loop = GetComponent<OverlayLoopVideoBackground>();
            if (loop == null)
            {
                loop = gameObject.AddComponent<OverlayLoopVideoBackground>();
            }
            ...
            loop.Bind(
                ...
                backgroundVideo,
                backgroundSprite,
                new Color(0.22f, 0.06f, 0.34f, 1f));
        }

        private void OnCardClicked(TreasureRewardSO reward)
        {
            if (_resolved || reward == null)
            {
                return;
            }
        ...
        private ChoiceCard EnsureCard(int index)
        {
            ...
            var go = HologramChoiceCardChrome.Create($"ChoiceCard_{index}", (RectTransform)transform, false);
            ApplyCardRect(go.transform as RectTransform, 0f);
        ...
        {
            public ChoiceCard(Transform root, Button button, Text kind, Text title, Text body)
            {
            ...
            public void Bind(TreasureRewardSO reward, Action<TreasureRewardSO> onClicked)
            {
                if (Kind != null)
                {
                    Kind.text = reward != null ? reward.KindLabel : string.Empty;
                }

                if (Title != null)
                {
                    Title.text = reward != null ? reward.Title : string.Empty;
                }

                if (Body != null)
                {
                    Body.text = reward != null ? reward.Description : string.Empty;
                }
                ...
                Button.interactable = reward != null;
                if (reward != null)
                {
                    Button.onClick.AddListener(() => onClicked?.Invoke(reward));
                }
            }
        }
    }
