...
namespace FracturedChorus.Editor
{
    public static class TimelineHierarchyBuilder
    {
        ...
        public static PartyStatusBarUIView BuildPartyStatusBar(Transform canvasTransform)
        {
            ...
            barRect.anchorMax = new Vector2(0f, 1f);
            barRect.pivot = new Vector2(0f, 1f);
            barRect.anchoredPosition = new Vector2(16f, -16f);
            barRect.sizeDelta = PartyStatusBarLayout.DefaultRootSize();

            var rowGo = CreateUiObject("CardsRow", barGo.transform);
            var cardsRow = rowGo.GetComponent<RectTransform>();
            StretchFull(cardsRow);

            var layout = rowGo.AddComponent<HorizontalLayoutGroup>();
            layout.spacing = PartyStatusBarLayout.DefaultCardSpacing;
            ...
            var template = BuildPartyCard("PartyCardTemplate", cardsRow.transform);
            template.gameObject.SetActive(false);

            barGo.AddComponent<PartyStatusBarSyncBinder>();
            var ui = barGo.AddComponent<PartyStatusBarUIView>();
            SetField(ui, "cardsRow", cardsRow);
            SetField(ui, "cardTemplate", template);
            SetField(ui, "preserveSceneLayout", true);
            ui.WireReferences();
            return ui;
        }

        private static PartyUnitCardView BuildPartyCard(string name, Transform parent)
        {
            var cardGo = CreateUiObject(name, parent);
            var cardRect = cardGo.GetComponent<RectTransform>();
            cardRect.sizeDelta = new Vector2(PartyStatusBarLayout.DefaultCardWidth, PartyStatusBarLayout.DefaultCardHeight);

            var layoutElement = cardGo.AddComponent<LayoutElement>();
            layoutElement.preferredWidth = PartyStatusBarLayout.DefaultCardWidth;
            layoutElement.preferredHeight = PartyStatusBarLayout.DefaultCardHeight;
            layoutElement.flexibleWidth = 0f;
            layoutElement.flexibleHeight = 0f;

            var frameGo = CreateUiObject("Frame", cardGo.transform);
            StretchFull(frameGo.GetComponent<RectTransform>());
            var frameImage = frameGo.AddComponent<Image>();
            frameImage.color = new Color(0.08f, 0.12f, 0.28f, 0.94f);
            frameImage.raycastTarget = false;
            EnsureUiSprite(frameImage);

            var avatarRootGo = CreateUiObject("AvatarRoot", cardGo.transform);
            var avatarRootRect = avatarRootGo.GetComponent<RectTransform>();
            avatarRootRect.anchorMin = new Vector2(0.5f, 1f);
            avatarRootRect.anchorMax = new Vector2(0.5f, 1f);
            avatarRootRect.pivot = new Vector2(0.5f, 1f);
            avatarRootRect.anchoredPosition = new Vector2(0f, -6f);
            avatarRootRect.sizeDelta = new Vector2(68f, 68f);

            var avatarGo = CreateUiObject("Avatar", avatarRootGo.transform);
            StretchFull(avatarGo.GetComponent<RectTransform>());
            var avatarImage = avatarGo.AddComponent<Image>();
            avatarImage.color = new Color(0.25f, 0.28f, 0.35f, 1f);
            avatarImage.preserveAspect = true;
            avatarImage.raycastTarget = false;
            EnsureUiSprite(avatarImage);

            var elementGo = CreateUiObject("ElementIcon", avatarRootGo.transform);
            var elementRect = elementGo.GetComponent<RectTransform>();
            elementRect.anchorMin = new Vector2(1f, 1f);
            elementRect.anchorMax = new Vector2(1f, 1f);
            elementRect.pivot = new Vector2(1f, 1f);
            elementRect.anchoredPosition = new Vector2(4f, 4f);
            elementRect.sizeDelta = new Vector2(24f, 24f);
            var elementImage = elementGo.AddComponent<Image>();
            elementImage.color = new Color(0.95f, 0.82f, 0.25f, 1f);
            elementImage.raycastTarget = false;
            elementImage.enabled = false;
            EnsureUiSprite(elementImage);

            var hpBarGo = CreateUiObject("HpBar", cardGo.transform);
            var hpBarRect = hpBarGo.GetComponent<RectTransform>();
            hpBarRect.anchorMin = new Vector2(0f, 0f);
            hpBarRect.anchorMax = new Vector2(1f, 0f);
            hpBarRect.pivot = new Vector2(0.5f, 0f);
            hpBarRect.anchoredPosition = new Vector2(0f, 6f);
            hpBarRect.sizeDelta = new Vector2(-12f, 10f);
            var hpBg = hpBarGo.AddComponent<Image>();
            hpBg.color = new Color(0.04f, 0.06f, 0.1f, 0.95f);
            ...
            hpFillImage.color = new Color(0.25f, 0.88f, 0.35f, 1f);
            ...
            EnsureUiSprite(hpFillImage);

            var card = cardGo.AddComponent<PartyUnitCardView>();
            SetField(card, "frame", frameImage);
            SetField(card, "avatar", avatarImage);
            SetField(card, "elementIcon", elementImage);
            SetField(card, "hpBackground", hpBg);
            SetField(card, "hpFill", hpFillImage);
            SetField(card, "preserveSceneImages", true);
            card.WireReferences();
            return card;
        }

        private static void EnsureUiSprite(Image image)
        {
            if (image != null && image.sprite == null)
            {
                image.sprite = UiSpriteUtil.White();
            }
        }
